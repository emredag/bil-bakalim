  ...
  try {
    ...
    for (let i = 0; i < wordSets.length; i++) {
      ...
      for (let length = 4; length <= 10; length++) {
        ...
        if (count !== 2) {
          log(
            `❌ FAILED: Player ${i + 1} has ${count} words of length ${length}, expected 2`,
            'red'
          );
          return false;
        }
...
async function testCategoryValidation(categoryId: number) {
  ...
  try {
    ...
    log(
      `Single mode (1 player): ${singleValid ? '✓ Valid' : '❌ Invalid'}`,
    ...
    log(
      `Multi mode (2 players): ${multiValid ? '✓ Valid' : '❌ Invalid'}`,
